...
void setupBLE() {
  ...
  advertisementData.setManufacturerData(
      std::string(reinterpret_cast<const char*>(MANUFACTURER_DATA),
  ...
  scanResponseData.setCompleteServices(
      BLEUUID((uint16_t)0x180A));  // Device Information Service
...
void loop() {
  ...
  // Warte bis zum nächsten Advertisement (1000ms Intervall - 50ms bereits
  // abgelaufen)
