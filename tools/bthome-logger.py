...
BTHOME_OBJECTS = {
    ...
    0x02: {'name': 'Temperatur', 'factor': 0.01, 'unit': '°C', 'signed': True, 'size': 2},
    ...
    0x07: {'name': 'Masse (lbs)', 'factor': 0.01, 'unit': 'lbs', 'signed': True, 'size': 2},
...
def print_header():
    ...
    print(f"{Colors.YELLOW}Filter: Geräte mit '{DEVICE_NAME_PREFIX}' im Namen{Colors.RESET}")
...
def advertisement_callback(device: BLEDevice, advertisement_data: AdvertisementData):
    ...
    if BTHOME_COMPANY_ID not in advertisement_data.manufacturer_data:
        ...
        for company_id, data in advertisement_data.manufacturer_data.items():
            ...
            print(f"{Colors.GRAY}[{format_timestamp()}]{Colors.RESET} "
    ...
    # RSSI
    rssi_color = Colors.GREEN if advertisement_data.rssi > -70 else Colors.YELLOW if advertisement_data.rssi > -85 else Colors.RED
    print(f"  {Colors.GRAY}RSSI:{Colors.RESET} {rssi_color}{advertisement_data.rssi} dBm{Colors.RESET}")
    ...
    if parsed:
        # Device Info
        encrypted_str = f"{Colors.RED}verschlüsselt{Colors.RESET}" if parsed['encrypted'] else f"{Colors.GREEN}unverschlüsselt{Colors.RESET}"
        print(f"  {Colors.GRAY}BThome:{Colors.RESET} {parsed['version']} ({encrypted_str})")
        ...
        if parsed['values']:
            ...
            for val in parsed['values']:
                ...
                print(f"    {Colors.BOLD}{Colors.MAGENTA}• {val['name']}{Colors.RESET}: "
